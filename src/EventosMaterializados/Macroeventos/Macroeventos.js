...
export default function Macroeventos(props) {
  ...
  useEffect(() => {
    const fetchdata = async () => {
        const resultRol = await fetch(process.env.REACT_APP_API_URL + "/macroevento/", {
          method: "GET",
          headers: {
  ...
  return (
    <>
      <Row className="mb-3 mt-3">
        <Col md={12}>
          <h1 className="titulo">Macroeventos</h1>
        </Col>
      </Row>
      <Row
      ...
      >
        ...
        >
          {showBotonEditar ? (
            <Link to={{
              pathname: "EditarMacroevento",
              state: {idmacro : selected[0]},
            }}>
              <Button
            ...
            <Table className={"text"} stickyHeader aria-label="sticky table">
              ...
              <TableBody>
                {dataBusqueda
                  ...
                  .map((row, index) => {
                    const isItemSelected = isSelected(row.idmacroevento);
                    return (
                      <StyledTableRow
                        key={row.idmacroevento}
                        hover
                        onClick={(event) =>
                          handleClick(event, row.idmacroevento)
                        }
                      ...
                      >
                        ...
                        <StyledTableCell component="th" scope="row">
                          {row.idmacroevento !== null ? row.idmacroevento : null}
                        </StyledTableCell>
                        <StyledTableCell align="left">
                          {row.compania !== null
                            ? row.compania
