...
export default function ModalAsociarMacroriesgos(props) {
  ...
  useEffect(() => {
    try {
      axios
        .get(process.env.REACT_APP_API_URL + "/macroriesgos/", {
          headers: {
          ...
          let filteredlist = data.filter(
            (element) =>
              !props.dataMacroriesgos.some(
                (item) => item.idmacroriesgos === element.idmacroriesgos
              )
          );
  ...
  const Asociar = () => {
    if (selected[0]) {
      const selectedData = data.filter(
        (macroriesgo) => macroriesgo.idmacroriesgos == selected[0]
      );
      props.dataMacroriesgos.push(selectedData[0]);
      setSelected([]);
      props.onHide();
    } else {
    }
