    ...
    try {
      axios
        ...
        .then((response) => {
          ...
          let apriori = data.filter((item) => item.cluster !== null);

          let filteredlist = apriori.filter(
            (element) =>
  ...
  const Asociar = () => {
    if (selected[0]) {
      const selectedData = data.filter(
        (item) => item.idsegmentos == selected[0]
      );

      props.dataSegmentos.push(selectedData[0]);
      setSelected([]);
      props.onHide();
    } else {
    }
                  ...
                  {data.map((row) => {
                    ...
                    return (
                      <StyledTableRow
                        key={row.idsegmentos}
                        hover
                        onClick={(event) => handleClick(event, row.idsegmentos)}
