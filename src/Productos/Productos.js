...
export default function Productos(props) {
  ...
  const editar = (event) => {
    localStorage.setItem("idProducto", selected[0]);
  };
  ...
  async function buscar(e) {
    ...
    var search = data.filter((item) => {
      if (
        String(item.idprod).includes(e.target.value.toLowerCase()) ||
            ...
            <Table className={"text"} stickyHeader aria-label="sticky table">
              ...
              <TableBody>
                {dataBusqueda
                  ...
                  .map((row, index) => {
                    const isItemSelected = isSelected(row.idprod);
                    return (
                      <StyledTableRow
                        key={row.idprod}
                        hover
                        onClick={(event) => handleClick(event, row.idprod)}
                      ...
                      >
                        ...
                        <StyledTableCell align="left">
                          {row.estado_prod ? "Activo" : "Inactivo"}
                        </StyledTableCell>
                      </StyledTableRow>
                    );
                  })}
