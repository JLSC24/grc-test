...
export default function Proveedores(props) {
  ...
  const editar = (event) => {
    localStorage.setItem("idProveedor", selected[0]);
  };
  ...
  async function buscar(e) {
    ...
    var search = data.filter((item) => {
      if (
        String(item.ID_SAP).includes(e.target.value.toLowerCase()) ||
        String(item.NIT).includes(e.target.value.toLowerCase()) ||
            ...
            <Table className={"text"} stickyHeader aria-label="sticky table">
              ...
              <TableHead className="titulo">
                <TableRow>
                  ...
                  <StyledTableCell>ID SAP</StyledTableCell>
